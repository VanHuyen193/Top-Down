...
public class WhipWeapon : ProjectileWeapon
{
    ...
    protected override bool Attack(int attackCount = 1)
    {
        ...
        {
            Debug.LogWarning("Projectile prefab has not been set");
            ActivateCooldown(true);
            return false;
        }

        // If no projectile prefab assigned, set the weapon on cooldown
        ...
        // Otherwise, caculate the angle and offset of our spawned projectile.
        // Then, if <currentSpawnCount> is even (i.e. more than 1 projectile),
        // we flip the direction of the spawn.
        float spawnDir = Mathf.Sign(movement.lastMoveVector.x) * (currentSpawnCount % 2 != 0 ? -1 : 1);
        ...
        currentAttackCount++;
        if(currentAttackCount > 1 && currentSpawnCount % 2 == 0)
        {
